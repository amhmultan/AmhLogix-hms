        ...
        <main>
            <div class="container bg-white shadow-md rounded my-6 px-5 py-4">
                ...
                <form method="POST" action="{{ route('admin.dosages.update',$dosage->id) }}" enctype="multipart/form-data">
                    ...
                    <p class="h3 text-danger"><strong><em>Update <span class="text-success">Dosage</span></em></strong></p>
                    <hr />
                    <div class="row mt-5">

                        <div class="col-md-4">
                            <label for="name" class="text-gray-700 font-black">Dosage Title:</label>
                            <input id="name" type="text" name="name" value="{{ old('name',$dosage->name) }}" class="form-control" />
                        </div>
                        <div class="col-md-4">
                            ...
                            <select class="border border-gray-300 rounded-full text-gray-600 h-10 pl-5 pr-10 bg-white hover:border-gray-400 focus:outline-none appearance-none" name="status">
                                <option value="0" {{ old('status', $dosage->status) == 0 ? 'selected' : '' }}>Active</option>
                                <option value="1" {{ old('status', $dosage->status) == 1 ? 'selected' : '' }}>Deactive</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <textarea name="description" id="description" placeholder="Enter Description" class="form-control" rows="5">{{ old('description',$dosage->description) }}</textarea>
