<x-app-layout>
    ...
    <script>
        ...
        function addRow() {
            ...
            row.innerHTML = `
        <td class="border p-1">
            <select name="items[${rowIndex}][fk_product_id]"
                    class="product-select w-full border rounded p-1"
                    required>
            </select>
        </td>
        ...
        <td class="border p-1">
            <input type="text"
                   ...
                   class="batch_no border rounded p-1 w-full" readonly>
        </td>

        <td class="border p-1">
            <input type="date"
                   name="items[${rowIndex}][expiry_date]"
                   class="expiry_date border rounded p-1 w-full" readonly>
        </td>

        <td class="border p-1">
            <input type="number"
                   name="items[${rowIndex}][quantity]"
                   class="quantity border rounded p-1 w-full"
                   min="1"
                   value="1">
        </td>
        ...
        <td class="border p-1">
            <input type="number"
                   ...
                   class="unit_price border rounded p-1 w-full"
                   value="0" readonly>
        </td>

        <td class="border p-1">
            <input type="text"
                   class="line-total border rounded p-1 w-full bg-gray-100"
                   readonly>
        </td>

        <td class="border p-1 text-center">
            <button type="button"
                    class="remove-item text-red-600">
                X
            </button>
        </td>
    `;
            ...
            $(row).find('.product-select').select2({
                ...
                ajax: {
                    url: '{{ route("admin.products.saledata") }}',
                    dataType: 'json',
                    delay: 250,
            ...
            $(row).find('.product-select').on('select2:select', function(e) {
                ...
                currentRow.find('.batch_no').val(selected.batch_no || '');
                currentRow.find('.expiry_date').val(selected.expiry_date || '');
                currentRow.find('.unit_price').val(selected.unit_price || 0);
        ...
        function calculateAmounts() {
            ...
            let taxPercent = parseFloat(document.getElementById('tax_percentage')?.value || 0);

            let afterDiscount = grossTotal - discount;
            ...
            document.getElementById('grossAmount').value = grossTotal.toFixed(2);
            document.getElementById('taxAmount').value = taxAmount.toFixed(2);
            document.getElementById('netAmount').value = netAmount.toFixed(2);

            document.getElementById('gross_amount_input').value = grossTotal.toFixed(2);
            document.getElementById('tax_amount_input').value = taxAmount.toFixed(2);
            document.getElementById('net_amount_input').value = netAmount.toFixed(2);
        }
    </script>
    @endpush
</x-app-layout>